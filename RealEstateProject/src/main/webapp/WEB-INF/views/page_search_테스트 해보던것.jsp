...
// 마커 추가 시간 체크
			...
			<%
			  ...
			  for (SaleTO saleItem : saleList) {
				//아파트를 제외하도록 추가한 부분
				  ...
				  } else if (!saleItem.getSale_type().equals("P")){
			%>
			...
			for (let i = 0; i < saleLats.length; i++) {
				saleMarkers.push({ 
					position: new naver.maps.LatLng(saleLats[i], saleLons[i]), 
					message: saleTitleArray[i]
			...
			for (let i = 0; i < stationLats.length; i++) {
				stationMarkers.push({ 
					position: new naver.maps.LatLng(stationLats[i], stationLons[i]), 
					message: stationTitleArray[i]
				});

			}
			
// 타임 체크
			console.log("docuemnt ready 전: " + ( new Date() - markerstart ));
			$(document).ready(function(){
// document ready 시간 체크	
				let readystart = new Date();
				
// init geocoder 전 시간 체크
				let geocodebefore = new Date();
				// 검색어를 받아오는 부분
				let keyword = '${keyword}';
...
// 여기 이후에 문제 발생
// geocodebefore
				console.log( "geocodebefore : " + (new Date() - geocodebefore));

//gecocodestart
				let geocodestart = new Date();		
...
// markerstart 문제의 코드 
				...
				let test5 = 0;
				let test6 = 0;
				// 폐기 (오래 걸리기에 미리 만들어 두고 수정하며 사용해보기
			   ...
			   /* saleMarkers.forEach(markerInfo => {
			        //const marker = new naver.maps.Marker({
			        // test1 마커를 새로 만들기
			        let test1start = new Date();
			        const marker = new naver.maps.Marker({
			            position: markerInfo.position,
			            map: null, // 초기에는 지도에 표시하지 않음
			        });
					test1 += (new Date() - test1start);

					// test2 마커 정보에 실제 마커 객체 저장
					let test2start = new Date()
			        markerInfo.marker = marker; // 마커 정보에 실제 마커 객체 저장
			        test2 += (new Date() - test2start);
			        
			        // test3
			        let test3start = new Date()
			        const infoWindow = new naver.maps.InfoWindow({
			            content: markerInfo.message
			        });
			        test3 += (new Date() - test3start);

			        // test4
			        let test4start = new Date();
			        infoWindows.push(infoWindow);
					test4 += (new Date() - test4start);
			        
			        // test5
			        let test5start = new Date();
			        naver.maps.Event.addListener(marker, 'click', function() {
			        	polygon.setMap(null);
			            infoWindows.forEach(window => window.close());
			        ...
			        test5 += (new Date() - test5start);
			        
			        // 마커들 저장
			        // test6
			        let test6start = new Date();
			        markers.push(marker);
			        test6 += (new Date() - test6start);
			    }); */
/* 			    console.log( "test1 time:" + test1 );	// 6482
			    console.log( "test2 time:" + test2 );	// 12
			    console.log( "test3 time:" + test3 );	// 2077
			    console.log( "test4 time:" + test4 );	// 9
			    console.log( "test5 time:" + test5 );	// 160
			    console.log( "test6 time:" + test6 );	// 10 */
// markerstart 측정
				console.log("markerstart: " + (new Date() - markerstart));
...
// 여기 이전에 문제 발생
			   ...
			   stationMarkers.forEach(markerInfo => {
				...
				console.log( "geocodestart: " + (new Date() - geocodestart));
				
// documet ready 시간 체크	
			console.log( "document ready 후 시간: " + (new Date() - readystart));
			});
		</script>
<!-- 	<style>
